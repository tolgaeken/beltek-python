...
def sil(hesap):
    global hesaplar
    silmeOnay = input(f"Kullanıcı adı '{hesaplar[hesap][3]}' olan kullanıcıyı silmek istediğinize emin misiniz?Evet ise 'e' yazınız\n").lower()
...
while True:
    ...
    match(menu):
        case 1:
            ...
            kullaniciAdi = input("Kullanıcı Adı Giriniz\n")
            parolaMenu = int(input("""
            1 - Rastgele parola üret
            2 - Yeni parola gir
            """))
            match(parolaMenu):
                case 1:
                    parolaUzunluk = int(input("Rastgele oluşturulacak parolanın uzunluğunu giriniz\n"))
                    parola = parolauret(parolaUzunluk)
                    print(f"Parolanız : {parola}")
                case 2:
                    parola = input("Parola giriniz\n")
                    print(f"Parolanız : {parola}")
